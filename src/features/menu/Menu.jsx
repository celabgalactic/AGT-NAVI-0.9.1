...
import { getNode } from '../tooltip/tooltipSlices';
...
export function Menu() {
  ...
  const selectedNode = useSelector(getNode)

  useEffect(() => {
    if (systemQuery) {
      const q = systemQuery.trim().toLowerCase();
      const match = systemList.find(
        f => f.regionID == regionID && f.name && f.name.toLowerCase() === q
      );
      if (match && (!selectedNode || selectedNode.glyphs !== match.glyphs)) {
        dispatch(changeSystem(match));
      }
    }
  }, [systemQuery, selectedNode, systemList, regionID, dispatch]);
        ...
        <ul>
          {regionMatches.map(r => {
            ...
            return (
              <li 
                key={r.id} 
                onClick={e => dispatch(changeRegion(r))}
