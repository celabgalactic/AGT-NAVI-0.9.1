    ...
    if (galaxyID > 0) {
      ...
      clustersMap = Object.keys(regions).reduce((res, k) => {
        ...
        const region = {
          ...
          systemCount,
          priority: regions[k].priority || '',
          priorityRank: regions[k].priorityRank || 0
        }
        res[id].regions.push(region)

        // Track the highest priority rank on the cluster and assign its color
        if (!res[id].priorityRank) {
          res[id].priorityRank = 0;
          res[id].priority = '';
        }
        if (region.priorityRank > res[id].priorityRank) {
          res[id].priorityRank = region.priorityRank;
          res[id].priority = region.priority;
        }

        const PRIORITY_COLORS = {
          1: 'rgb(17,115,75)',   // Low
          2: 'rgb(255,225,0)',  // Medium
          3: 'rgb(255,147,0)'   // High
        }
        if (res[id].priorityRank > 0) {
          res[id].color = PRIORITY_COLORS[res[id].priorityRank];
        } else {
          res[id].color = undefined;
        }
